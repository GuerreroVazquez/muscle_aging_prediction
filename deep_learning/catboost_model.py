...
class optimize_classification:
    ...
    def objective(self, trial):
        ...
        'verbose': 0,  # Fixed verbosity
        'od_type': 'Iter',  # Early stopping type
        'od_wait': 15,  # Early stopping patience
        'l2_leaf_reg': trial.suggest_float('l2_leaf_reg', 0.0, 10.0),  # L2 regularization coefficient
        ...
        y_pred = regressor.predict(self.X_test)
        # Calculate rmse
        auc = accuracy_score(self.y_test, y_pred)
        return auc
    
def optimize_data(dataset):
    opt = optimize(train_data = dataset, n_trials=20)
    params = opt.optimize()
    return params

def optimize_data_classification(dataset):
    opt = optimize_classification(train_data = dataset, n_trials=20)
    params = opt.optimize()
    return params